...
def list_all(request, *args, **kw):
    ...
    sorting = request.GET.get('sort', request.session.get("sort_adm_list", sort_default))

    if sorting not in columns:
        sorting = sort_default
    if sorting.startswith('-') and sorting[1:] not in columns:
        sorting = sort_default
    ...
    page_num = 1
    page_asked = int(request.session.get('page_adm_list', 1))

...
class InMemoryBacklink(object):
    ...
    def __unicode__(self):
        return "<Backlink %s [%s] QUERY_STRING: %s>" % (self.index, self.path, self.QUERY_STRING)
...
def backlink_setter(view_func):
    ...
    So when you click the item, it will load a view which will not be a backlink_setter.
    Then, the middleware for each request will place a "backlink" in the request, so you can
    ...
    def _wrapped_view(request, *view_args, **view_kw):
        if not request.is_ajax():
            ...
            backlinks.append(InMemoryBacklink(request, view_func, view_args, view_kw))

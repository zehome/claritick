...
class FormSecurityChecker(object):
    ...
    def _security_filter(self, user, formName=None, security={}):
        ...
        self._security_default_level = self._security_settings.get(
                                            "__default__",
                                            settings.SECURITY["DEFAULT_LEVEL"])
        for fname in self.fields.keys():
            required_level = self._security_settings.get(
                                        fname, self._security_default_level)
    ...
    def filter_querydict(cls, user, querydict):
        """
        This method will filter querydict,
        not letting user pass invalid data to the view.
        """
        userlevel = user.my_userprofile.get_security_level()
        security_settings = settings.SECURITY.get(cls.__name__, {})
        security_default_level = security_settings.get(
                            "__default__", settings.SECURITY["DEFAULT_LEVEL"])
...
class HostForm(df.ModelForm, FormSecurityChecker):
    ...
    class Meta:
        ...
        fields = ("site", "hostname", "ip", "os", "rootpw",
                  "supplier", "model", "type", "location",
                  "serial", "inventory", "date_start_prod", "date_end_prod",
                  "status", "commentaire")
    ...
    def __init__(self, user, ip, *args, **kwargs):
        ...
        if self.is_new:
            self.fields['os'].queryset = \
                self.fields['os'].queryset.filter(depleted=False)
            self.fields['supplier'].queryset = \
                self.fields['supplier'].queryset.filter(depleted=False)
    ...
    def log_action(self, action, instance=None):
        ...
        log = HostEditLog(host=instance, username=self.user.username,
                          ip=self.user_ip, action=action, message=message)
        log.save()
        return log

    def save(self, force_insert=False, force_update=False,
             commit=True, POST={}, prefix=""):
        """
        Write the Host diff in HostVersion.
        It creates AdditionnalFieldForm and save it
        """
        ...
        for elem in self.changed_data:
            if elem != "type":
                host_changes += \
                    u"Le champ Host.%s est passé de <%s> à <%s>\n" % (
                ...
                if self.initial["type"]:
                    ...
                    old_type = 'None'
                host_changes += \
                    u"Le champ Host.type est passé de <%s> à <%s>\n" % (
        ...
        if (host_changes or fields_changes):
            ...
            HostVersion(host=host_changes, additionnal_fields=fields_changes,
                        log_entry=log).save()
        return (inst, extra_fields)
...
def _init_field(field, initial):
    ...
    if field.data_type == '1':
        ...
        return df.MultipleChoiceField(choices=enumerate(field.default_values),
                                      **args)
...
class AdditionnalFieldForm(df.Form):
    def __init__(self, *args, **kwargs):
        ...
        # Détermine la liste des champs et leurs valeurs courrante
        # (défaut puis établi si existant)
        if blank:
            c_fields = None
        else:
            c_fields = \
                dict((v.id, v.default_values) for v in (self.avail_fields))
        if host:
            host_fields = dict((addf.field.id, addf.value)
                               for addf in host.additionnalfield_set.all()
                               if addf.field.id in c_fields.keys())
            self.host_fields = \
                dict(('val_%s' % (k,), v) for k, v in host_fields.iteritems())
            if not blank:
                c_fields.update(dict((k, v)
                                for k, v in host_fields.iteritems()
                                if k in c_fields.keys()))
        # Ajoute les champs dojango au formulaire

        for field in self.avail_fields:
            self.fields['val_%s' % (field.id, )] = \
                _init_field(field, c_fields and c_fields[field.id])
    ...
    def get_data(self):
        ...
        return dict((k, self.cleaned_data[k])
                ...
                if not((isinstance(v, df.ChoiceField) and
                       self.cleaned_data[k] == '-1')))
...
class ParamAdditionnalFieldAdminForm(df.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'data_type': MyDojoFilteringSelect(
                            attrs={'onchange': 'typeChanged(this);'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'instance' in kwargs:
            ...
            if inst.data_type in ('1', '2', '4', '5'):
                ...
                for i, e in ((i, "choice%s_val" % (str(i + 1).rjust(2, '0')))
                                                   for i in range(16)):
                    try:
    ...
    def clean(self):
        ...
        if (api_key and ParamAdditionnalField.objects.filter(
                            api_key__exact=api_key)
                            .filter(host_type__exact=host_type)
                            .exclude(pk=self.instance.id)):
            self._errors["api_key"] = \
                self.error_class(["Doit être unique par type d'hôte"])
    ...
    def save(self, commit=True):
        ...
        if normal_fields.get(cd['data_type'], False):
            ...
            dv = [cd[e] for e in ("choice%s_val" % (str(i).rjust(2, '0'),)
                                                    for i in range(1, 16))
                                                    if cd[e]]
...
class SearchHostForm(df.Form, ModelFormTableMixin, FormSecurityChecker):
    ...
    cheat_1 = df.CharField(max_length=1, label='',
                widget=df.TextInput(attrs={'class': 'dijitHidden'}),
                required=False)
    ...
    # these ModelChoiceFields are initialised twice.
    # once forvalidation and once after filtering
    ...
    os = df.ModelChoiceField(queryset=OperatingSystem.objects.all(),
        widget=MyDojoFilteringSelect(), empty_label='',
        required=False, label=u'OS')
    supplier = df.ModelChoiceField(queryset=Supplier.objects.all(),
        widget=MyDojoFilteringSelect(), empty_label='',
        required=False, label=u'Fournisseur')
    status = df.ModelChoiceField(queryset=HostStatus.objects.all(),
        widget=MyDojoFilteringSelect(), empty_label='',
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        ordered_clients = \
            sort_queryset(Client.objects
                          .filter(id__in=(c.id for c in user.clients)))
        self.fields['site'].choices = \
            [('', '')] + [(c.id, unicode(c)) for c in ordered_clients]
        self._security_filter(user)

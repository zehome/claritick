    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for etat_data in [(1,'Nouveau',0),(2,'Actif',1),(3,'En attente',2),(4,'Fermé',3)]:
                if not tmod.State.objects.filter(pk = int(etat_data[0])).count():
            ...
            for prio_data in r.fetch_row(maxrows = 0):
                if not tmod.Priority.objects.filter(pk = int(prio_data[0])).count():
            ...
            for pro_data in r.fetch_row(maxrows = 0):
                if not tmod.Project.objects.filter(pk = int(pro_data[0])).count():
            ...
            for cl_data in rows:
                parent = None
                if not cmod.Client.objects.filter(pk = int(cl_data[0])).count():
            ...
            for cat_data in r.fetch_row(maxrows = 0):
                if not tmod.Category.objects.filter(label = recoder(cat_data[0])).count():
            ...
            for tic_data in rows:
                if tmod.Ticket.objects.filter(pk = int(tic_data[0])).count():
                    continue
                cpt += 1
                try:

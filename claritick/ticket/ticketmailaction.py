...
def send_emails():
    ...
    ticket_tmas = TicketMailAction.objects \
        .filter(date__lte=now - datetime.timedelta(seconds=email_delay)) \
        .values('ticket') \
        .annotate(datemax=Max('date'))
    for ticket_tma in ticket_tmas:
        ...
        reasons = []
        tmas = TicketMailAction.objects.filter(ticket__pk=ticket_id).order_by('date')
        for tma in tmas:
            reasons.extend(tma.reasons)

        # Suppression des doublons
        reasons_unique = list(set(reasons))
